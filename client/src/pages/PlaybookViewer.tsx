  ...
  return (
    ...
    <svg ref={svgRef} viewBox={`0 0 ${W} ${H}`} className="w-full rounded-xl border border-cyan-500/20 block" style={{ background: '#0a0a0f' }}
      ...
      <text x={W - 6} y={losY - 3} fill="#22d3ee" fontSize={9} fontFamily="monospace" textAnchor="end" fontWeight="bold" opacity={0.9}>
        {t("playbook:losLabel")} {Math.round(yardFromY(losY))}
      </text>
  ...
  if (plays.length === 0) {
    return (
      <div className="text-center py-20" data-testid="playbook-empty">
        ...
        <p className="text-muted-foreground text-sm">{t("playbook:noPlays")}</p>
      </div>
    );
  }
      ...
      <div className="space-y-4 max-w-3xl mx-auto" data-testid="playbook-viewer-detail">
        <div className="flex items-center justify-between">
          ...
          >
            ...
            <span>{t("playbook:backToFolder", { folder: folderName === 'Általános' ? t("playbook:generalFolder") : folderName })}</span>
          </button>
        ...
        <div className="text-center">
          ...
          <p className="text-xs text-cyan-400/40">{folderName === 'Általános' ? t("playbook:generalFolder") : folderName}</p>
        </div>
        ...
        {current.data.note && (
          <div className="bg-black/30 border border-cyan-500/15 rounded-lg px-3 py-2" data-testid="text-play-note">
            <p className="text-[10px] text-cyan-400/40 font-mono uppercase tracking-wider mb-0.5">{t("playbook:note")}</p>
  ...
  if (view.mode === 'plays') {
    ...
    return (
      <div className="space-y-4 max-w-4xl mx-auto" data-testid="playbook-viewer-plays">
        ...
        >
          <ArrowLeft className="w-3.5 h-3.5" />
          <span>{t("playbook:folders")}</span>
        </button>

        <div className="flex items-center gap-2">
          <FolderOpen className="w-5 h-5 text-cyan-400" />
          <h2 className="text-lg font-bold text-foreground">{view.folder === 'Általános' ? t("playbook:generalFolder") : view.folder}</h2>
          <span className="text-xs text-cyan-400/40">{t("playbook:playCount", { count: folderPlays.length })}</span>
        </div>

        {folderPlays.length === 0 ? (
          <p className="text-center text-muted-foreground/50 text-sm py-10">{t("playbook:noPlaysInFolder")}</p>
        ) : (
  ...
  return (
    <div className="space-y-4 max-w-3xl mx-auto" data-testid="playbook-viewer">
      <div className="text-center mb-2">
        <h2 className="text-lg font-bold text-foreground">{t("playbook:title")}</h2>
        <p className="text-xs text-cyan-400/40">{t("playbook:folderCount", { plays: plays.length, folders: sortedFolders.length })}</p>
      </div>
      ...
      <div className="grid grid-cols-2 sm:grid-cols-3 gap-3">
        {sortedFolders.map(([folder, folderPlays]) => (
          ...
          >
            ...
            <p className="font-bold text-sm text-foreground group-hover:text-cyan-400 transition-colors truncate">{folder === 'Általános' ? t("playbook:generalFolder") : folder}</p>
            <p className="text-[11px] text-cyan-400/30 mt-0.5">{t("playbook:playCount", { count: folderPlays.length })}</p>
          </button>
        ))}
      </div>
    </div>
