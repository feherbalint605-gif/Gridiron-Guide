...
export default function PlaybookEditor() {
  ...
  const setFolderAccessMutation = useMutation({
    ...
    onError: () => toast({ title: t("playbook:folderAccessSaveError"), variant: "destructive" }),
  });
  ...
  const saveMutation = useMutation({
    mutationFn: async () => {
      if (!playName.trim()) throw new Error(t("playbook:playNameRequired"));
      ...
      try {
        ...
        throw new Error(err?.message || t("playbook:saveError"));
      }
    },
    onSuccess: (data) => {
      queryClient.invalidateQueries({ queryKey: ['/api/playbook'] });
      if (!editingId && data?.id) setEditingId(data.id);
      toast({ title: editingId ? t("playbook:playUpdated") : t("playbook:playSaved") });
    },
  ...
  const deleteMutation = useMutation({
    ...
    onSuccess: (_, deletedId) => {
      ...
      toast({ title: t("playbook:playDeleted") });
    },
  });
    ...
    <div className="flex gap-4 h-full" data-testid="playbook-editor">
        ...
        return (
          ...
          >
            <div className="px-3 py-1.5 text-[10px] text-cyan-400/50 uppercase tracking-widest font-mono border-b border-cyan-500/10 mb-1">
              {t("playbook:folderAccessTitle", { folder: folderMenuOpen === 'Általános' ? t("playbook:generalFolder") : folderMenuOpen })}
            </div>
            <button
            ...
            >
              <Unlock className="w-3.5 h-3.5" /> {t("playbook:everyoneCanSee")}
            </button>
            {coachTeams.map(team => (
              <button
                key={team.id}
            ...
            {coachTeams.length === 0 && (
              <p className="px-3 py-2 text-[10px] text-muted-foreground italic">{t("playbook:noTeamsCreated")}</p>
            )}
          </div>
        );
      })()}
      <div className="w-48 shrink-0 flex flex-col gap-2">
        <div className="flex items-center justify-between">
          <span className="text-xs font-mono uppercase text-cyan-400 tracking-widest">{t("playbook:title")}</span>
          <div className="flex items-center gap-1">
            <button onClick={() => { setShowNewFolder(true); setNewFolderName(''); }} className="text-cyan-400/60 hover:text-cyan-300" title={t("playbook:newFolder")} data-testid="button-new-folder">
              <FolderPlus className="w-4 h-4" />
            </button>
            <button onClick={newPlay} className="text-cyan-400 hover:text-cyan-300" title={t("playbook:newPlay")} data-testid="button-new-play">
        ...
        {showNewFolder && (
          <div className="flex gap-1">
            <Input
              ...
              placeholder={t("playbook:folderNamePlaceholder")}
        ...
        <div className="space-y-0.5 max-h-[420px] overflow-y-auto pr-1">
          {plays.length === 0 && (
            <p className="text-xs text-muted-foreground/50 text-center py-6">{t("playbook:noSavedPlays")}</p>
          )}
          {(() => {
                ...
                <div key={folder} data-testid={`folder-${folder}`}>
                  <div className={cn("flex items-center gap-1 w-full px-1.5 py-0.5 rounded transition-colors",
                    ...
                    >
                      ...
                      <span className="flex-1 text-left truncate">{folder === 'Általános' ? t("playbook:generalFolder") : folder}</span>
                    ...
                    <div className="shrink-0" data-folder-menu>
                      <button
                        ...
                        title={isLocked ? `${t("playbook:locked")} ${assignedTeam?.name ?? '?'}` : t("playbook:everyoneCanSee")}
      ...
      <div className="flex-1 flex flex-col gap-3 min-w-0">
        <div className="flex flex-wrap items-center gap-2 bg-black/40 border border-cyan-500/20 rounded-lg px-3 py-2">
          ...
          <div className="flex gap-1">
            ...
            >
              <MousePointer className="w-3 h-3" /> {t("playbook:moveTool")}
            </button>
            <button
            ...
            >
              <Pen className="w-3 h-3" /> {t("playbook:routeTool")}
            </button>
            {playMode === 'defense' && (
              <button
              ...
              >
                <Shield className="w-3 h-3" /> {t("playbook:zoneTool")}
              </button>
            )}
          </div>
          {tool === 'zone' && playMode === 'defense' && (
            <>
              ...
              <div className="flex items-center gap-1 bg-black/40 rounded border border-red-500/20 p-0.5">
                <span className="text-[9px] text-red-400/50 px-1">{t("playbook:shapeLabel")}</span>
                {([['ellipse', Circle, t("playbook:ellipse")], ['rect', Square, t("playbook:rectangle")], ['freehand', Spline, t("playbook:freehand")]] as [ZoneShape, React.FC<{className?: string}>, string][]).map(([shape, Icon, label]) => (
              ...
              {selectedZoneId && (
                <button onClick={deleteSelectedZone}
                  ...
                  <Trash2 className="w-3 h-3" /> {t("playbook:deleteZone")}
                </button>
              )}
            </>
          )}
          ...
          <div className="flex items-center gap-1 flex-wrap">
            <span className="text-[10px] text-cyan-400/50 uppercase tracking-widest">{t("playbook:addPlayer")}</span>
          ...
          <div className="flex items-center gap-2">
            <span className="text-[10px] text-cyan-400/50 uppercase">{t("playbook:losLabel")}</span>
            <select
              value={losY}
              onChange={e => {
          ...
          {selectedId && (
            <div className="ml-auto flex items-center gap-1">
              ...
              >
                ...
                {play.playerNotes?.[selectedId] ? t("playbook:editNote") : t("playbook:addNote")}
              </button>
              <button
              ...
              >
                <Trash2 className="w-3 h-3" />
                {play.routes.some(r => r.playerId === selectedId) ? t("playbook:deleteRoute") : t("playbook:deletePlayer")}
              </button>
            </div>
          )}
        </div>
        ...
        <div className="relative" ref={containerRef}>
              ...
              >
                <div className="bg-black/95 border rounded-lg px-2.5 py-1.5 shadow-lg max-w-[200px]"
                  ...
                  <div className="flex items-center gap-1 mb-0.5">
                    ...
                    <span className="text-[9px] text-white/40">{t("playbook:note")}</span>
                  </div>
            ...
            <div className="absolute top-2 right-2 flex items-center gap-2">
              {isDrawingZone ? (
                <div className="px-2 py-1 bg-red-500/20 text-red-300 text-[10px] rounded border border-red-500/30 font-bold">
                  {zoneTool === 'freehand' ? t("playbook:drawingHint") : t("playbook:dragZoneHint")}
                </div>
              ) : selectedId ? (
                <div className="px-2 py-1 bg-black/80 text-red-400/70 text-[10px] rounded border border-red-500/20">
                  {t("playbook:zoneDrawHint", { shape: zoneTool === 'ellipse' ? t("playbook:ellipse") : zoneTool === 'rect' ? t("playbook:rectangle") : t("playbook:freehand") })}
                </div>
              ) : (
                <div className="px-2 py-1 bg-black/80 text-red-400/50 text-[10px] rounded border border-red-500/20">
                  {t("playbook:selectPlayerFirst")}
                </div>
              )}
            ...
            <div className="absolute top-2 right-2 flex flex-col items-end gap-1.5">
              ...
              {!isDrawing && (
                <div className="flex items-center gap-1 bg-black/80 rounded border border-cyan-500/20 p-0.5">
                  <span className="text-[9px] text-cyan-400/50 px-1">{t("playbook:lineLabel")}</span>
                  {([['solid', t("playbook:solid")], ['dashed', t("playbook:dashed")], ['dotted', t("playbook:dotted")]] as [RouteLineStyle, string][]).map(([style, label]) => (
              ...
              {!isDrawing && (
                <div className="flex items-center gap-1 bg-black/80 rounded border border-cyan-500/20 p-0.5">
                  <span className="text-[9px] text-cyan-400/50 px-1">{t("playbook:endLabel")}</span>
                  {([
                    ['arrow', t("playbook:arrow"), (
          ...
          {showRouteMenu && selectedPlayer && isReceiver(selectedPlayer.type) && (
            ...
            >
              <div className="px-3 py-1.5 border-b border-cyan-500/15">
                <span className="text-[10px] font-mono text-cyan-400 uppercase tracking-widest">{t("playbook:routeTree")}</span>
              </div>
              ...
              <div className="border-t border-cyan-500/15 mt-1 pt-1">
                ...
                >
                  ...
                  <span className="text-foreground/90">{t("playbook:customRoute")}</span>
                </button>
          ...
          {(() => {
            ...
            return (
              <div className="absolute bottom-3 left-3 z-40 bg-black/85 border border-cyan-500/25 rounded-lg px-3 py-2 flex flex-col gap-1 min-w-[160px]">
                <div className="flex items-center justify-between">
                  <span className="text-[10px] text-cyan-400/70 font-mono uppercase tracking-widest">{t("playbook:tempo")}</span>
                  <span className="text-[11px] font-bold text-cyan-300">{speed === 1 ? t("playbook:tempoNormal") : speed < 1 ? t("playbook:tempoSlow") : t("playbook:tempoFast")} {speed}×</span>
                </div>
                <input
                  type="range"
                  min={0.25}
          ...
          <button
            ...
            title={t("playbook:playAnimationTitle")}
          >
            {isAnimating ? (
        ...
        <div className="flex flex-col gap-1.5">
          <div className="flex gap-2">
            <Input
              ...
              placeholder={t("playbook:playNamePlaceholder")}
            ...
            >
              ...
              {saveMutation.isPending ? t("common:saving") : editingId ? t("playbook:update") : t("playbook:save")}
            </Button>
          </div>
          <div className="flex gap-2 items-center">
            ...
            >
              {[...new Set([playFolder, ...plays.map(p => p.folder || 'Általános')])].sort().map(f => (
                <option key={f} value={f} className="bg-black text-white">{f === 'Általános' ? t("playbook:generalFolder") : f}</option>
              ))}
            </select>
          </div>
          <textarea
            ...
            placeholder={t("playbook:playNotePlaceholder")}
            rows={2}
            ...
            <div className="fixed inset-0 z-[100] flex items-center justify-center bg-black/60" onClick={() => setEditingPlayerNote(null)}>
              <div className="bg-[#0f1117] border rounded-xl p-4 w-72 shadow-2xl" style={{ borderColor: cfg.color + '40' }} onClick={e => e.stopPropagation()}>
                <div className="flex items-center gap-2 mb-3">
                  ...
                  <span className="text-sm font-bold text-white">{cfg.label} — {t("playbook:note")}</span>
                </div>
                <textarea
                  autoFocus
                  value={playerNoteText}
                  onChange={e => setPlayerNoteText(e.target.value)}
                  placeholder={t("playbook:playerNotePlaceholder")}
                  rows={3}
